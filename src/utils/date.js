...
export function getQuarterMountsToToday(year, month) {
  if (monthInQuarter(month) === 1) {
    ...
    return new Date(year, month - 2, 0).getDate() + new Date(year, month - 1, 0).getDate() + new Date().getDate()
  }
}
...
export const getlastTwoYearsMountsToToday = (year, month) => {
  return getLastYearMounts(year - 1) + getLastYearMounts(year) + getYearMountsToToday(year, month)
}
